...
from .utils.table_utils import handleSpecial

def populateVarStatsTable(df:pd.DataFrame, expected_cols = 9, verbose = False):
    if expected_cols != df.shape[1]:
        raise ValueError(f'The input data frame for the table must have {expected_cols} columns.')

    # rename columns for display
    df.columns = ['Var Name', 'Var Class', 'Mean', 'Median', 'Min', 'Max',
                  'SD', 'Obs', 'Missing Obs']
    # subset only necessary columns
    df = df[['Var Name', 'Var Class', 'Mean', 'SD']]
